...
function watcher() {
    ...
    gulp.watch(path.watch.less, less);//для всех задачь чтобы сразу заливать на сервер
...
const build = gulp.series(reset, mainTasks, gulp.parallel(watcher, server));//gulp.parallel(watcher, server) добавил
